...
const Release = (release: Props) => {
  ...
  const contributingStudies = _uniq(release.authors.map(({ study }) => study)).filter(
    (name) => name !== 'Admin and Analysis Team'
  )
  const contributors = _sortBy(release.authors, ({ name }) => _last(name.split(' '))).map(({ name }) => name)
  ...
  return (
    <Container disableGutters={true}>
      ...
      <Grid container={true} alignItems={'center' as 'center'} spacing={2} marginTop={1}>
