...
import {  ExpandMore, ChevronRight, Info, Announcement, Book, Notes, Share, BarChart, CloudDownload, Computer, People } from "@material-ui/icons";
...
const navItems: NavItem[] = [
    ...
    children: [
      {
        ...
        icon: Book,
        children: [
          {href: "/blog/2020-11-24-covid-19-hgi-results-for-data-freeze-4-october-2020/", label: "Scientific Summary (R4)"},
  ...
  },
  {
    ...
    children: [
      ...
      {href: "https://app.covid19hg.org", label: "Data Browser"},
      {
  ...
  const renderNavItem = useCallback((item: NavItem, index: number) => {
    ...
    if ("children" in item) {
      ...
      const linkElem = item.href.startsWith("/") ? (
        <GatsbyLink to={item.href} className={classes.link} > {item.label} </GatsbyLink>
      ) : (
        <Link href={item.href} className={classes.link}>{item.label}</Link>
      )
      labelContent = (
        <div className={classes.treeItemLabelRoot}>
          {linkElem}
